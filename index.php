...
$sql = "SELECT * FROM workers" . ($where?" WHERE ".implode(" AND ",$where):'') . " ORDER BY $sort_col";
...
:root{--bg:#0f1117;--surface:#181c27;--surface2:#1e2333;--border:#2a3045;--accent:#4f8ef7;--accent2:#7c5cfc;--text:#e8eaf0;--muted:#8891aa;--card-bg:#181c27;--navbar-h:62px;--sidebar-w:270px;--danger:#f87171;--success:#4ade80;}
body{background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;min-height:100vh;}
</style>
<style><?php include "navbar.css"; ?></style>
<style>
.layout{display:flex;padding-top:var(--navbar-h);}
.sidebar{width:var(--sidebar-w);min-width:var(--sidebar-w);position:fixed;top:var(--navbar-h);left:0;bottom:0;background:var(--surface);border-right:1px solid var(--border);overflow-y:auto;padding:24px 20px;display:flex;flex-direction:column;gap:20px;}
...
.main{margin-left:var(--sidebar-w);flex:1;padding:28px 28px 40px;min-height:calc(100vh - var(--navbar-h));}
...
<div class="layout">
    <aside class="sidebar">
        ...
        <div class="filter-group">
            ...
            <select id="f-profession">
                ...
                <?php $profs=mysqli_query($conn,"SELECT DISTINCT profession FROM workers WHERE approved=1 ORDER BY profession"); while($p=mysqli_fetch_assoc($profs)){ $sel=(($_GET['profession']??'')===$p['profession'])?'selected':''; echo "<option value='{$p['profession']}' $sel>{$p['profession']}</option>"; } ?>
            </select>
        </div>
        <div class="filter-group">
            ...
            <select id="f-location">
                ...
                <?php $locs=mysqli_query($conn,"SELECT DISTINCT location FROM workers WHERE approved=1 ORDER BY location"); while($l=mysqli_fetch_assoc($locs)){ $sel=(($_GET['location']??'')===$l['location'])?'selected':''; echo "<option value='{$l['location']}' $sel>{$l['location']}</option>"; } ?>
            </select>
        </div>
        <div class="filter-group"><label>Min Experience (yrs)</label><input type="number" id="f-exp" min="0" placeholder="Any" value="<?= htmlspecialchars($_GET['min_exp']??'') ?>"></div>
        <div class="filter-group"><label>Min Rating</label><input type="number" id="f-rating" step="0.1" min="0" max="5" placeholder="Any" value="<?= htmlspecialchars($_GET['min_rating']??'') ?>"></div>
        <div class="filter-group">
    ...
    <main class="main">
        ...
        <?php if($count===0): echo '<div class="empty-state"><p>No workers match your filters.</p></div>'; endif; ?>
        <?php while($row=mysqli_fetch_assoc($result)):
            $name=$row['name']; $prof=htmlspecialchars($row['profession']); $exp=htmlspecialchars($row['experience']);
            $avail=$row['availability']??''; $photo=!empty($row['photo'])?htmlspecialchars($row['photo']):'';
        ...
        <a class="worker-card" href="worker_details.php?id=<?= $id ?>">
            <div class="card-img-wrap">
                <?php if($photo): ?><img src="uploads/<?= $photo ?>" alt="<?= htmlspecialchars($name) ?>" loading="lazy"><?php else: ?><div class="avatar-fallback"><?= $initials ?></div><?php endif; ?>
            ...
            <div class="card-info">
                ...
                <div class="card-meta">
                    ...
                    <?php if($avail==='available'): ?><span class="badge avail">● Available</span><?php endif; ?>
                </div>
            </div>
        </a>
        <?php endwhile; ?>
...
// Shared panel JS (used by navbar.php includes)
